...
import ru.org.myapp.cadence.samples.SignalWorkflow;
  ...
  public void getGreeting(SampleMessage sampleMessage) {
    ...
    while (!cancel) {
      ...
      if (!this.greetingMsg.isEmpty()) {
        ...
        // A cadence execution cannot receive infinite number of signals due to history limit
    ...
    logger.info("cadence canceled");
  }

  @Override
  public void waitForGreeting(String greeting) {
    if (cancel) {
      logger.info("signal cadence failed because it is already cancelled");
      return;
    }
